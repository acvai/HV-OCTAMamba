import shutil
import torch
from dataset_w_DataAugment import writer
from dataset_w_DataAugment import *
from utils import *
from settings_benchmark_w_DA import *
...
for name_model in models:
    ...
    for name_dataset in all_dataset:
        ...
        ### batch_size=2: the training goes faster 
        trainLoader = DataLoader(dataset=dataset['train'], batch_size=2, shuffle=True, drop_last=False, num_workers=8)
        ### batch_size=4: the training goes slower
        # trainLoader = DataLoader(dataset=dataset['train'], batch_size=4, shuffle=True, drop_last=False, num_workers=8)
        ...
        for epoch in range(NUM_MAX_EPOCH):
            ...
            if dice > bestResult['dice']:
                # Here is the best result on the validation set:
