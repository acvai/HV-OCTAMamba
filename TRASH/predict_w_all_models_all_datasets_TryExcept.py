...
from our_model.HV_OCTAMamba import *
...
datasets = ["OCTA500_3M", "OCTA500_6M"]  #datasets = ["OCTA500_3M", "OCTA500_6M", "ROSSA"]
    ...
    for dataset in datasets:
        try:
            ...
            if not os.path.exists(model_path):
                print(f"❌ Skipping {model_name} on {dataset} — model weights not found at {model_path}")
                continue

            if not os.path.isdir(image_folder):
                print(f"❌ Skipping {dataset} — image folder not found: {image_folder}")
                continue
            ...
            print(f"🔥 Error in model: {model_name}, dataset: {dataset} — {str(e)}")
